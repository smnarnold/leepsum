...
class LoremIpsum {
  ...
  generateSentences() {
    ...
    for (let x = 0; x < structure.length; x++) {
      ...
      if (firstChar === '*') {
        if (Math.random() * 6 < 1) type = type.slice(1);
        else continue;
      } else if (firstChar === '?') {
        let obj = '';

        if (type.slice(1) === 'actions') obj = this.getSection(type.slice(1), protagonist);
        else obj = this.getSection(type.slice(1));
      ...
      if (literal) sentenceArr.push(type);
      else sentenceArr.push(this.getRandomWord(type));
    }
  ...
  getSection(originalType, preset = {}) {
    ...
    if (typeObj.hasOwnProperty('options')) {
      ...
      if (typeof type === 'string') obj = this.getPartial(type, preset);
  ...
  getPartial(type = 'persons', preset = {}) {
    ...
    for (let x = 0; x < data[type].structure.length; x++) {
      ...
      if (obj[category]['required'] >= Math.random()) textArr.push(this.getArrRandomNoRepeat(obj[category]['values']));
      else if (typeof obj[category]['default'] !== 'undefined') textArr.push(obj[category]['default']);
    }
  ...
  getRandomWord(typeOfWord) {
    ...
    if (!typeof this.usedDictionnary[typeOfWord] !== 'undefined') this.usedDictionnary[typeOfWord] = [];
    ...
    if (this.usedDictionnary[typeOfWord].indexOf(wordIndex) === -1) {
      ...
      if (this.usedDictionnary[typeOfWord].length > 4)
        // make sure the same word dosn't get repeated too quickly
